...
int main(void){
    ...
    unsigned char * restrict img = stbi_load("bird.jpg", &width,&height,&channels, 0);
    ...
    unsigned char * restrict new_img = malloc(img_size);
    if (new_img == NULL){
    ...
    unsigned char * restrict ycbcr_img = new_img;
    if (ycbcr_img == NULL){
        printf("Error, no image found.\n");
        exit(1);
    }
    ...
    for (unsigned char *i = ycbcr_img, *ni = rgb_img; i !=ycbcr_img+img_size-channels; i +=channels, ni +=channels){
        ...
        *(ni+channels) = (uint8_t) Max(0, Min(255, (4194304 * (*i+channels) + (5872026 * (*(i+2+channels)-128))) >>22));                                               //R
        *(ni+1+channels) =(uint8_t) Max(0, Min(255, (4194304 * (*i+channels) - (2982150 * ((*(i+2+channels)-128))) - (1438646 * (*(i+1+channels)-128))) >>22));                 //G
        *(ni+2+channels) =(uint8_t) Max(0, Min(255, (4194304 * (*i+channels) + (7402947 * (*(i+1+channels)-128))) >>22));                                          //B
    }
